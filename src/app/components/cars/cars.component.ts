...
showUpdate(id:any){
  ...
  if(this.carEdit){
    ...
    if(lip && cab &&cam && cad && red && img){
      ...
      if(carImage){
        img.setAttribute("href",'http://localhost:4000/upload/'+carImage);
        img.setAttribute("target","popup");
        img.setAttribute("onclick","window.open('http://localhost:4000/upload/"+carImage+"','popup','width=600,height=600'); return false;");
      }
    }
  }
